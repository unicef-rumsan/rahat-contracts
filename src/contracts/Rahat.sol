...
contract Rahat is AccessControl, AbstractERC20Func, Multicall {
    ...
    function setAsBankedBeneficiary(
        uint256 _phone,
        bool _isBanked
    ) external OnlyAdmin {
    ...
    function issueERC20ToBeneficiary(
        ...
        require(
            registry.exists(findHash(_phone)),
    ...
    function transferTokenToVendorByAdmin(
        uint256 _phone,
    ...
    function transferCashToVendor(
        address _vendorAddress,
        uint _amount
    ) public OnlyAdmin {
        require(
            registry.exists(findHash(_vendorAddress)),
            "vendor wallet not registered."
        );
        cashToken.approve(
            registry.id2Address(findHash(_vendorAddress)),
            _amount
        );
    }

    function _transferCashFromVendor(
        address _vendorAddress,
        uint256 _phone,
        uint _amount
    ) private {
        RahatWallet vendorWallet = RahatWallet(
            registry.id2Address(findHash(_vendorAddress))
        );
        vendorWallet.transferToken(
            address(cashToken),
            registry.id2Address(findHash(_phone)),
            _amount
        );
    }

    function _verifyVendorCashBalance(
        address _vendorAddress,
        uint256 _amount
    ) private view {
        uint balance = cashToken.balanceOf(
            registry.id2Address(findHash(_vendorAddress))
        );
        require(
            balance >= _amount,
            "Vendor does not have enough cash balance."
        );
    }

    function claimTokenForProject(
        address _token,
        address _from,
        bytes32 _projectId,
        uint256 _amount
        ...
        )
    {
        if (address(cashToken) == address(0)) {
            ...
            cashAllowance = cashToken.allowance(
                address(_allowanceFrom),
                address(this)
            );
    ...
    function beneficiaryBalance(
        uint _phone
    )
        public
        view
        returns (address walletAddress, uint cashBalance, uint tokenBalance)
    {
